...
public class EnemyFSM : MonoBehaviour
{
    [Header("Pursuit")]
    [SerializeField]
    private float targetRecognitionRange = 40;          // 인식 범위 (이 범위 안에 들어오면 "Pursuit" 상태로 변경)
    [SerializeField]
    private float pursuitLimitRange = 50;              // 추적 범위 (이 범위 바깥으로 나가면 "Wander"상태로 변경)
 
    [Header("Attack")]
    [SerializeField]
    private GameObject enemyBulletPrefab;              // 발사체 프리펩
    [SerializeField]
    private Transform enemyBulletSpawnPoint;           // 발사체 생성 위치
    [SerializeField]
    private float attackRange = 30;                    // 공격 범위(이 범위 안에 들어오면 "Attack"상태로 변경)
    [SerializeField]
    ...
    private EnemyMemoryPool enemyMemoryPool;    // 적 메모리 풀(적 오브젝트 비활성에 사용)
    private Animator enemyanimator;                  // 적 애니메이션 제어
    ...
    private IEnumerator Idle()
    {
        ...
        {
            // "대기" 상태일 때 하는 행동
            // 타겟과의 거리에 따라 행동 선택(배회, 추격, 근거리 공격)
    ...
    {
        float wanderRadius = 100;    // 현재 위치를 원점으로 하는 원의 반지름
